 ...
 *
 *    File name: promisify-utils.js
 *    Last modified: 7/29/23, 8:20 PM
...
module.exports = {
    createPromisifyProcess: async function({ execCommand, messOnStart, messOnEnd, stage, allStages, options }) {
        ...
        const spinner = this.createAndStartSpinner({ stage, allStages, messages: messOnStart });
        try {
    ...
    checkIfDockerContainerIsRunning: async function({ containerName, stage, allStages }) {
        ...
        const spinner = this.createAndStartSpinner({
            stage, allStages, messages: `Checking, if container ${containerName.cyan} is running`,
        });
        try {
