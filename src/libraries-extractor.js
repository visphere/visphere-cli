 ...
 *
 *    File name: libraries-extractor.js
 *    Last modified: 7/29/23, 10:12 PM
...
const promisifyUtils = require("./helpers/promisify-utils");
...
const staticReplacements = [
    { name: "@ngx-translate/core", repoUrl: "https://github.com/ngx-translate/core", license: "MIT", env: "runtime" },
    { name: "@ngx-translate/http-loader", repoUrl: "https://github.com/ngx-translate/core", license: "MIT", env: "runtime" },
];
...
async function readAllScannedPackagesDependencies() {
    ...
    try {
        for (const packagePath of scanningPackages) {
            ...
            if (devDepSection) resultsArray.push(...Object.keys(devDepSection).map(name => ({ name, env: "development" })));
        }
    } catch (err) {
    ...
    utils.stopSucessSpinner(spinner,
...
async function connectWithApiAndGenerateOutputObject(allLibrariesNames) {
    ...
    try {
        for (const { name, env } of allLibrariesNames) {
            ...
            if (!replacer) {
                ...
                outputArray.push({ name, repoUrl, license: data["license"], env: env });
                utils.revalidateSpinnerContent(spinner, currentStage, allStages,
        ...
        utils.stopErrorSpinner(spinner, `Unable to fetch additional informations for scanned libraries`,
            currentStage, allStages);
        throw new Error(err);
    }
    ...
    utils.stopSucessSpinner(spinner, `Fetched additional informations for ${coverageInfo} scanned libraries`,
        currentStage++, allStages);
    return outputArray;
}
...
async function saveContentToLibrariesFiles(outputLibsArray) {
    ...
    try {
        ...
        await fs.promises.writeFile(outputJsonPath, "");
        await fs.promises.appendFile(outputJsonPath, "[\n");
        let libraryIndex = 0;
        for (const { name, repoUrl, license, env } of outputLibsArray) {
            const comma = ++libraryIndex === outputLibsArray.length ? "" : ",";
            await fs.promises.appendFile(outputJsonPath,
                `\t{ "name": "${name}", "repoUrl": "${repoUrl}", "license": "${license}", "env": "${env}" }${comma}\n`);
        }
        await fs.promises.appendFile(outputJsonPath, "]\n");
        utils.revalidateSpinnerContent(spinner, currentStage, allStages,
    ...
    utils.stopSucessSpinner(spinner, `Successfully saved data in output files (${currentOutput}/2)`,
        currentStage++, allStages);
}
